...
//   useEffect(() => {
//     fetch("http://localhost:8080/api/Beauty")
//       .then((res) => res.json())
...
const Beauty = () => {
  ...
  const updateSlidesPerView = useCallback(() => {
    ...
    setIsMobile(width < 768);
    // Reset to first slide on resize if needed
    if (currentSlide > BeautyData.length - slides) {
      setCurrentSlide(Math.max(0, BeautyData.length - slides));
    }
  }, [currentSlide, BeautyData.length]);
  ...
  const maxSlide = Math.max(0, BeautyData.length - slidesPerView);
    ...
    fetch("http://localhost:8080/api/wishlist", {
      ...
      .then(async (res) => {
        if (res.ok) {
          ...
          setWishlistItems((prev) => new Set([...prev, product.id])); // Sync local state
          setNotification({
            type: "info",
            message: "Item already in wishlist",
          });
          ...
          setNotification({
            type: "error",
            message: err.error || err.message || "Failed to add to wishlist",
          });
          setTimeout(() => setNotification(null), 2000);
        }
      })
  ...
  return (
    <div className="w-full py-4 sm:py-6 md:py-8 px-3 sm:px-4 md:px-6 lg:px-8 bg-gradient-to-b from-gray-50 to-white">
                  ...
                  >
                    ...
                    <div className="absolute bottom-3 right-3 flex gap-2 sm:opacity-0 sm:group-hover:opacity-100 transition-all duration-300">
                      <button
                        ...
                        className="w-7 h-7 sm:w-8 sm:h-8 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center shadow-md hover:bg-white active:scale-95 transition-all"
                      >
                        <Heart
                          size={14}
                          className={`sm:w-4 sm:h-4 ${wishlistItems.has(product.id)
      ...
      <div className="mt-4 sm:mt-6 flex flex-col items-center">
        ...
        <div className="w-full text-center">
          <button className="w-full sm:w-auto px-6 py-3 bg-gradient-to-r from-purple-600 to-pink-600 text-white font-semibold rounded-lg hover:from-purple-700 hover:to-pink-700 transition-all duration-300 shadow-lg hover:shadow-xl active:scale-95 text-sm sm:text-base">
            View All Products ({BeautyData.length})
          </button>
          <p className="text-xs text-gray-500 mt-2">
            Free shipping on orders over $50
          </p>
